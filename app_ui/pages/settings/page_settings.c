...
#define SETTINGS_LEFT_W             42
#define SETTINGS_LIST_X             44
#define SETTINGS_LIST_Y             8
#define SETTINGS_LIST_W             142
#define SETTINGS_EDITOR_X           188
#define SETTINGS_EDITOR_W           68
#define SETTINGS_ROW_W              136
#define SETTINGS_SWITCH_TRACK_X     112
#define SETTINGS_SWITCH_THUMB_OFF_X 114
#define SETTINGS_SWITCH_THUMB_ON_X  124
...
static ui_rect_t    s_root_left_bar;
static ui_label_t   s_root_title;
static ui_label_t   s_root_subtitle;
...
static ui_rect_t    s_section_left_bar;
static ui_rect_t    s_section_parent_tile;
static ui_rect_t    s_section_active_tile;
static ui_label_t   s_section_parent_label;
static ui_label_t   s_section_active_label;
...
static void set_row_style(settings_row_widgets_t *row, bool selected, bool faded, bool toggle_on) {
    ...
    row->switch_thumb.base.x = toggle_on ? SETTINGS_SWITCH_THUMB_ON_X : SETTINGS_SWITCH_THUMB_OFF_X;
}
...
static void sync_section_rows(bool animate_scroll) {
    ...
    ui_label_set_text(&s_section_active_label, section_desc->title);
...
static void sync_editor_panel(void) {
    ...
    ui_widget_set_visible(&s_editor_panel.base, s_editor_open);
    ui_widget_set_visible(&s_editor_head.base, s_editor_open);
    ui_widget_set_visible(&s_editor_title.base, s_editor_open);
    ui_widget_set_visible(&s_editor_mode.base, s_editor_open);
    ...
    ui_label_set_text(&s_editor_title, item->title);
...
static void open_editor(void) {
    s_editor_open = true;
    s_editor_panel.base.offset_x = 6.f;
    sync_section_rows(false);
    sync_editor_panel();
    ui_widget_animate(&s_editor_panel.base.offset_x, 6.f, 0.f, 180,
                      ui_ease_out_cubic, NULL, NULL);
}
...
static void on_root_appear(ui_page_t *p) {
    (void)p;
    s_root_content.offset_y = list_target_offset(s_root_selected, SETTINGS_ROOT_COUNT) + 6.f;
    s_root_title.base.alpha = 0.f;
    s_root_subtitle.base.alpha = 0.f;
    sync_root_page(false);
    ui_widget_animate(&s_root_content.offset_y, s_root_content.offset_y,
                      list_target_offset(s_root_selected, SETTINGS_ROOT_COUNT), 240,
                      ui_ease_out_cubic, NULL, NULL);
    ui_widget_animate(&s_root_title.base.alpha, 0.f, 1.f, 180,
                      ui_ease_out_cubic, NULL, NULL);
    ui_widget_animate(&s_root_subtitle.base.alpha, 0.f, 1.f, 220,
                      ui_ease_out_cubic, NULL, NULL);
}
...
static void on_section_appear(ui_page_t *p) {
    (void)p;
    close_editor();
    s_section_content.offset_y = list_target_offset(s_section_selected, current_item_count()) + 6.f;
    s_section_parent_label.base.alpha = 0.f;
    s_section_active_label.base.alpha = 0.f;
    sync_section_rows(false);
    ui_widget_animate(&s_section_content.offset_y, s_section_content.offset_y,
                      list_target_offset(s_section_selected, current_item_count()), 240,
                      ui_ease_out_cubic, NULL, NULL);
    ui_widget_animate(&s_section_parent_label.base.alpha, 0.f, 1.f, 180,
                      ui_ease_out_cubic, NULL, NULL);
    ui_widget_animate(&s_section_active_label.base.alpha, 0.f, 1.f, 220,
                      ui_ease_out_cubic, NULL, NULL);
}
...
static void on_root_action(ui_action_t action, const ui_event_t *raw, void *ctx) {
    ...
    switch (action) {
        ...
        case UI_ACTION_CONFIRM:
            ...
            close_editor();
            app_ui_push_id(APP_PAGE_SETTINGS_SECTION, UI_TRANS_SLIDE_LEFT, UI_TRANS_DURATION_MS);
            break;
...
static void build_root_page(void) {
    ...
    ui_rect_init(&s_root_left_bar, 0, 0, SETTINGS_LEFT_W, 64, UI_GRAY_DIM);
    ui_widget_add_child(&s_root_widget, &s_root_left_bar.base);

    ui_label_init(&s_root_title, 3, 18, SETTINGS_LEFT_W - 6, 8, "SET", &g_font,
                  UI_GRAY_WHITE, UI_GRAY_BLACK, true);
    s_root_title.align = UI_ALIGN_CENTER;
    ui_widget_add_child(&s_root_widget, &s_root_title.base);

    ui_label_init(&s_root_subtitle, 3, 30, SETTINGS_LEFT_W - 6, 8, "MENU", &g_font,
                  UI_GRAY_LIGHT, UI_GRAY_BLACK, true);
    s_root_subtitle.align = UI_ALIGN_CENTER;
    ui_widget_add_child(&s_root_widget, &s_root_subtitle.base);

    ui_widget_init(&s_root_content, SETTINGS_LIST_X, SETTINGS_LIST_Y, SETTINGS_LIST_W, 48, NULL);
...
static void build_section_page(void) {
    ...
    ui_rect_init(&s_section_left_bar, 0, 0, SETTINGS_LEFT_W, 64, UI_GRAY_DIM);
    ui_widget_add_child(&s_section_widget, &s_section_left_bar.base);

    ui_rect_init(&s_section_parent_tile, 3, 8, SETTINGS_LEFT_W - 6, 12, UI_GRAY_SHADOW);
    ui_widget_add_child(&s_section_widget, &s_section_parent_tile.base);

    ui_label_init(&s_section_parent_label, 3, 10, SETTINGS_LEFT_W - 6, 8, "SET", &g_font,
                  UI_GRAY_LIGHT, UI_GRAY_BLACK, true);
    s_section_parent_label.align = UI_ALIGN_CENTER;
    ui_widget_add_child(&s_section_widget, &s_section_parent_label.base);

    ui_rect_init(&s_section_active_tile, 3, 24, SETTINGS_LEFT_W - 6, 20, UI_GRAY_WHITE);
    ui_widget_add_child(&s_section_widget, &s_section_active_tile.base);

    ui_label_init(&s_section_active_label, 3, 30, SETTINGS_LEFT_W - 6, 8, "INPUT", &g_font,
                  UI_GRAY_BLACK, UI_GRAY_WHITE, true);
    s_section_active_label.align = UI_ALIGN_CENTER;
    ui_widget_add_child(&s_section_widget, &s_section_active_label.base);

    ui_widget_init(&s_section_content, SETTINGS_LIST_X, SETTINGS_LIST_Y, SETTINGS_LIST_W, 48, NULL);
    ui_widget_add_child(&s_section_widget, &s_section_content);

    for (i = 0; i < SETTINGS_MAX_ITEMS; ++i) {
        init_row_widgets(&s_section_rows[i], &s_section_content, i * SETTINGS_ROW_PITCH);
    }

    ui_rect_init(&s_editor_panel, SETTINGS_EDITOR_X, 0, SETTINGS_EDITOR_W, 64, UI_GRAY_BLACK);
    s_editor_panel.border_w = 1;
    s_editor_panel.border_gray = UI_GRAY_WHITE;
    ui_widget_add_child(&s_section_widget, &s_editor_panel.base);

    ui_rect_init(&s_editor_head, SETTINGS_EDITOR_X, 0, SETTINGS_EDITOR_W, 10, UI_GRAY_WHITE);
    ui_widget_add_child(&s_section_widget, &s_editor_head.base);

    ui_label_init(&s_editor_title, SETTINGS_EDITOR_X + 4, 1, 40, 8, "", &g_font,
                  UI_GRAY_BLACK, UI_GRAY_WHITE, true);
    ui_widget_add_child(&s_section_widget, &s_editor_title.base);

    ui_label_init(&s_editor_mode, SETTINGS_EDITOR_X + 44, 1, 16, 8, "", &g_font,
                  UI_GRAY_BLACK, UI_GRAY_WHITE, true);
    s_editor_mode.align = UI_ALIGN_RIGHT;
    ui_widget_add_child(&s_section_widget, &s_editor_mode.base);

    for (i = 0; i < SETTINGS_EDITOR_CHOICES; ++i) {
        ui_rect_init(&s_editor_choice_bg[i], SETTINGS_EDITOR_X + 6, 14 + i * 12, 56, 10, UI_GRAY_SHADOW);
        ui_widget_add_child(&s_section_widget, &s_editor_choice_bg[i].base);

        ui_label_init(&s_editor_choice_label[i], SETTINGS_EDITOR_X + 8, 15 + i * 12, 52, 8, "", &g_font,
                      UI_GRAY_WHITE, UI_GRAY_BLACK, true);
        s_editor_choice_label[i].align = UI_ALIGN_CENTER;
        ui_widget_add_child(&s_section_widget, &s_editor_choice_label[i].base);
    }

    ui_rect_init(&s_editor_switch_track, SETTINGS_EDITOR_X + 16, 20, 36, 10, UI_GRAY_DIM);
    ui_widget_add_child(&s_section_widget, &s_editor_switch_track.base);

    ui_rect_init(&s_editor_switch_thumb, SETTINGS_EDITOR_X + 18, 21, 14, 8, UI_GRAY_WHITE);
    ui_widget_add_child(&s_section_widget, &s_editor_switch_thumb.base);

    ui_label_init(&s_editor_state_label, SETTINGS_EDITOR_X + 8, 38, 52, 8, "", &g_font,
                  UI_GRAY_WHITE, UI_GRAY_BLACK, true);
    s_editor_state_label.align = UI_ALIGN_CENTER;
    ui_widget_add_child(&s_section_widget, &s_editor_state_label.base);

    ui_rect_init(&s_editor_bar_track, SETTINGS_EDITOR_X + 6, 20, 56, 10, UI_GRAY_DIM);
    ui_widget_add_child(&s_section_widget, &s_editor_bar_track.base);

    ui_rect_init(&s_editor_bar_fill, SETTINGS_EDITOR_X + 6, 20, 24, 10, UI_GRAY_WHITE);
    ui_widget_add_child(&s_section_widget, &s_editor_bar_fill.base);

    ui_label_init(&s_editor_value_label, SETTINGS_EDITOR_X + 4, 36, 60, 8, "", &g_font,
                  UI_GRAY_WHITE, UI_GRAY_BLACK, true);
    s_editor_value_label.align = UI_ALIGN_CENTER;
    ui_widget_add_child(&s_section_widget, &s_editor_value_label.base);
